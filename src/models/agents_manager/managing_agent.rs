...
mod tests {
    ...
    async fn test_managing_agent() {
        let user_request =
            "I need a website that tracks my daily activities and provides a summary report, it need to include nutritional information from api"
                .to_string();

        let mut managing_agent = ManagingAgent::new(user_request).await.unwrap();
